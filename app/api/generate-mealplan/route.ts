...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `
      ...
      Do not include any newlines within JSON string values.
    ...
    const response = await openai.chat.completions.create({
      model: "liquid/lfm-2.5-1.2b-instruct:free", // Ensure this model is accessible and suitable
      messages: [
        {
          role: "user",
          content: prompt,
    ...
    const cleanedContent = aiContent
      ...
      .replace(/[\r\n]+/g, ' ');   // Replace newlines with spaces (in case they're breaking strings)
